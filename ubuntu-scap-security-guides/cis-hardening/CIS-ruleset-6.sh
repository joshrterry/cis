...
rule-6.2.5()
{
    ...
    local v=$(awk -F'=' '/^\s*DIR_MODE=[0-9]{3,4}/{ print $2 }' ${adduser_conf})
    if [ -n "${v}" ]; then
        for u in ${v}; do
            # check if num passes valid mask
            if [ $((0${u} & 07027)) -ne 0 ]; then
    ...
    v=$(awk -F' ' '/^\s*HOME_MODE\s+[0-9]{3,4}/{ print $2 }' ${useradd_conf})
    if [ -n "${v}" ]; then
        for u in ${v}; do
            # check if num passes valid mask
            if [ $((0${u} & 07027)) -ne 0 ]; then
                sed -E -i 's/(^HOME_MODE\s+).*$/\10750/g' ${useradd_conf}
                break
            fi
        done
    else
        echo 'HOME_MODE           0750' >> ${useradd_conf}
    fi
}
